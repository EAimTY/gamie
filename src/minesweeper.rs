...
pub use impls::{Cell, CellStatus, Error, Game, Status};
    ...
    impl Game {
        ...
        pub fn click(&mut self, row: usize, col: usize, auto_flag: bool) -> Result<(), Error> {
            ...
            match self.board[row * self.width + col].status {
                ...
                CellStatus::Revealed => {
                    let adjacent_mine_count =
                        self.board[row * self.width + col].adjacent_mine_count;
...
pub mod rkyv {
    pub use super::impls::{
        ArchivedCell, ArchivedCellStatus, ArchivedError, ArchivedGame, ArchivedStatus,
        CellResolver, CellStatusResolver, ErrorResolver, GameResolver, StatusResolver,
    };
}
